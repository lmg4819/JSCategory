...
#import "NSObject+JSRuntime.h"
#import <objc/runtime.h>

@implementation NSObject (JSRuntime)
...
{
    return [[self class] js_propertyKeys];
}
...
{
    return [[self class] js_propertiesInfo];
}
...
+ (NSArray *)js_propertiesInfo
{
    ...
    for (int i = 0; i < propertyCount; i++)
    {
        [propertieArray addObject:({
            ...
            NSDictionary *dictionary = [self js_dictionaryWithProperty:properties[i]];
...
+ (NSArray *)js_propertiesWithCodeFormat
{
    ...
    NSArray *properties = [[self class] js_propertiesInfo];
...
- (NSArray *)js_instanceMethodList{
    return  [[self class] js_instanceMethodList];
}
...
{
    return [[self class] js_classMethodList];
}
...
+ (NSArray *)js_classMethodList
{
    ...
    return [metaClass js_instanceMethodList];
}
...
-(NSArray*)js_methodListInfo{
    ...
    for (int i = 0; i < count ; i++)
    {
        ...
        for (int index=0; index<argumentsCount; index++) {
            ...
            [arguments addObject:[[self class] js_decodeType:arg]];
        }
        
        NSString *returnTypeString =[[self class] js_decodeType:returnType];
        NSString *encodeString = [[self class] js_decodeType:encoding];
...
+ (NSArray *)js_instanceVariable
{
    ...
    for (int i = 0; i < outCount; i++) {
        NSString *type = [[self class] js_decodeType:ivar_getTypeEncoding(ivars[i])];
...
-(NSDictionary *)js_protocolList{
    return [[self class] js_protocolList];
}
+ (NSDictionary *)js_protocolList
{
...
- (void)js_setAssociateValue:(id)value withKey:(void *)key {
    objc_setAssociatedObject(self, key, value, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (void)js_setAssociateWeakValue:(id)value withKey:(void *)key {
...
- (id)js_getAssociatedValueForKey:(void *)key {
    return objc_getAssociatedObject(self, key);
}


#pragma mark -- help
+ (NSDictionary *)js_dictionaryWithProperty:(objc_property_t)property
{
...
+ (NSString *)js_decodeType:(const char *)cString
{
    ...
    } else
    {
        if ([[result substringToIndex:1] isEqualToString:@"^"]) {
            result = [NSString stringWithFormat:@"%@ *",
                      [NSString js_decodeType:[[result substringFromIndex:1] cStringUsingEncoding:NSUTF8StringEncoding]]];
        }
    }
    return result;
}
